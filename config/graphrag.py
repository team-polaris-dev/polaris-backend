...
# ── 스키마 가드 text-to-Cypher (graphrag/cypher_generator.py, cypher_executor.py) ──
# 손수 짠 plan-kind 분류기 대신 LLM이 그래프 스키마에서 read-only Cypher 를 생성하는 경로.
# 구조(관계/존재)만 다루고 지표 랭킹은 기존 structured_executor 가 처리한다(supported=false).
# 기본 OFF — 와이어링(search.py) 전까지 런타임 동작이 바뀌지 않도록 한다(fail-closed).
TEXT2CYPHER_ENABLED = os.environ.get("GRAPHRAG_TEXT2CYPHER", "0") not in ("0", "false", "False")
