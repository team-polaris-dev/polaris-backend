...
# 허브 우회 노이즈를 억제한다(HippoRAG 방식). networkx.pagerank(personalization=시드).
...
# ── 스키마 가드 text-to-Cypher (graphrag/text2cypher.py, cypher_executor.py) ──
# 공식 Neo4j Text2CypherRetriever 가 그래프 스키마에서 read-only Cypher 를 생성하는 엔진.
# 관계/구조/존재는 이 경로가, 재무 지표 랭킹은 그 위 value-add(cypher_executor.rank_results
# → MariaDB 결정적 SQL)가 담당한다. 기본 ON — 공식 리트리버가 1차 엔진. 생성/실행 실패는
# fail-closed(None) 로 기존 결정적 planner 경로로 폴백하므로 서버에서도 안전하다.
TEXT2CYPHER_ENABLED = os.environ.get("GRAPHRAG_TEXT2CYPHER", "1") not in ("0", "false", "False")
# 결정성 주입 시 ORDER BY 없는 쿼리에 붙일 LIMIT 상한.
TEXT2CYPHER_RESULT_LIMIT = int(os.environ.get("GRAPHRAG_TEXT2CYPHER_RESULT_LIMIT", "50"))
# EXPLAIN 실패 시 LLM 재호출(수리) 최대 횟수. 정적 가드 실패는 수리하지 않는다.
TEXT2CYPHER_REPAIR_MAX = int(os.environ.get("GRAPHRAG_TEXT2CYPHER_REPAIR_MAX", "1"))
