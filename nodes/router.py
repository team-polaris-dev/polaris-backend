...
2. 사용자의 일상어는 단어집에서 의도에 가장 맞는 표준 용어 "하나"로만 바꾸세요. 후보가 여러 개여도 나열하지 말고 하나만 고릅니다. 예) "돈 얼마 벌었어?" → "매출액"(O) / "매출액, 영업이익 등"(X). 단, 사용자가 명시하지 않은 연도·항목·기준은 임의로 지어내지 마세요.
3. 이전 맥락을 모르는 사람이나 DB 검색기라도 완벽하게 이해할 수 있는 하나의 독립적인 질문으로 작성하세요.
4. 부연 설명이나 인사말 없이 오직 '재구성된 질문' 자체만 텍스트로 출력하세요.
5. '등', '기타', '및', '예를 들어', '(A, B, C …)' 같은 나열·뭉뚱그림·예시 표현을 절대 붙이지 마세요. 사용자가 말하지 않은 항목으로 질문 범위를 넓히지 말고, 재구성 질문은 하나의 명확한 대상·지표만 담습니다.
