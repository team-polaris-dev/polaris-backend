"""비정형 관계 추출 적재 — SK하이닉스 2025년 3분기 분기보고서 (rcept 20251114001805, 187청크).

EXTRACTIONS = Claude(에이전트)가 대상 rcept 의 청크 본문을 하나씩 읽고 본문 근거로
판단한 엔티티·엣지. 적재는 extract_helpers 의 멱등 헬퍼로 수행.
원장은 rcept 전용 ledger/20251114001805.jsonl 에만 기록. 정형 표(재무·지분·계열사) 제외, 본문 근거 비정형만.

실행: cd db && PYTHONIOENCODING=utf-8 uv run python graph/extract_skhynix_q_20251114001805.py
"""
...
EXTRACTIONS: dict[str, dict] = {
    ...
    "7d9320db7a107e74": {  # 매출 품목 표: DRAM, NAND Flash 등 / 주요상표 SK하이닉스
        "entities": [
    ...
    "af069f9d2fbd4ef1": {  # 메모리 반도체: 당사는 DRAM과 플래시메모리 생산
        "entities": [],
        "edges": [
    ...
    # ── 연구개발 실적 표: UFS/ZUFS, Automotive SSD, CMM-DDR5(CXL) ─
    "c20e11423990a444": {  # R&D 실적: Mobile UFS/ZUFS, Automotive PCIe Gen4 SSD, CMM-DDR5(CXL)
        "entities": [
            (P, "ufs", "UFS"),
            (P, "zufs", "ZUFS"),
            (P, "ssd", "SSD"),
        ...
        "edges": [
            ...
            E("PRODUCES", ("org", SKH), ("ent", P, "zufs", "ZUFS"), 0.88),
    ...
    # ── 그래픽스 GDDR / 게임콘솔 / HBM 시황 ────────────────────
    "0232c485c5ea9c16": {  # GDDR(AI추론 non-HBM), 게임콘솔 협력, HBM
        "entities": [
            (P, "gddr", "GDDR"),
            (P, "hbm", "HBM"),
        ],
    ...
    # ── PC 메모리: 24/48GB DDR5 공급 ───────────────────────────
    "29d93ac201243b43": {  # PC 메모리: 24GB/48GB DDR5 공급 확대, CXL
        "entities": [
            (P, "ddr5", "DDR5"),
            (T, "cxl", "CXL"),
        ],
    ...
    "90300e51781267b3": {  # Automotive: 범용 DDR4/5, LPDDR4/5, Automotive Grade & HBM
        "entities": [
            (P, "lpddr5", "LPDDR5"),
        ],
        "edges": [
    ...
    "6cbb16612e535ec3": {  # QLC, PCIe Gen5, 엔터프라이즈 SSD, Solidigm 통합 시너지
        "entities": [
            (P, "qlc", "QLC"),
    ...
    # ── 경쟁요소: TSV 기술 ─────────────────────────────────────
    "23ddfb8929be7a4c": {  # DRAM TSV(Through Silicon Via) 기술 경쟁력
        "entities": [
    ...
    # ── HBM/ASIC 협력 (Big Tech) ───────────────────────────────
    "74b936a0bd8ab888": {  # HBM 시장 선도, Big Tech 기업과 ASIC 개발 협력
        "entities": [],
        "edges": [
    ...
    "25b48cb27fef932c": {  # 주요계약: Rambus 특허 크로스라이선스, Intel NAND 영업양수
        "entities": [],
        "edges": [
...
def run():
    ...
    print(f"=== SK하이닉스 2025 3분기 ({RCEPT}) 비정형 추출 결과 ===")
