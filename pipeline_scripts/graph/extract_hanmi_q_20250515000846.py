"""비정형 관계 추출 적재 — 한미반도체 2025년 1분기 분기보고서 (rcept 20250515000846, 125청크).

원장은 rcept 전용 ledger/20250515000846.jsonl 에만. 시작 시 기처리 청크 스킵,
대상 청크 전부 mark_processed(엣지 0개여도) → 누락 0. 정형 재무표 제외, 본문 근거 비정형만.

실행: cd db && PYTHONIOENCODING=utf-8 uv run python graph/extract_hanmi_q_20250515000846.py
"""
...
EXTRACTIONS: dict[str, dict] = {
    ...
    # 제조용 장비 라인업 + DUAL TC BONDER HBM 핵심
    "998a58a72a1ae4ec": {
        "entities": [
    ...
    # 시장여건·경쟁상황: DUAL TC BONDER 2017 SK하이닉스 공동개발, 6-SIDE INSPECTION
    "ce0f19e141ed4910": {
        "entities": [],
        "edges": [
            E("PRODUCES", ("org", HANMI), ("ent", P, "dual tc bonder", "DUAL TC BONDER"), 0.95),
    ...
    # DUAL TC BONDER HBM 핵심, 6-SIDE INSPECTION, FLIP CHIP/MULTI DIE/BIG DIE BONDER(2010~), micro SAW
    "92e3358b9aedf867": {
        "entities": [
    ...
    # BIG DIE BONDER, micro SAW/VISION PLACEMENT 세계1위, EMI Shield
    "38aba3f34c58337c": {
        "entities": [],
        "edges": [
    ...
    # micro SAW 국산화, EMI Shield
    "0a41e2c69cfa073c": {
        "entities": [],
        "edges": [
            E("PRODUCES", ("org", HANMI), ("ent", P, "micro saw", "micro SAW"), 0.9),
            E("PRODUCES", ("org", HANMI), ("ent", P, "vision placement", "VISION PLACEMENT"), 0.88),
            E("PRODUCES", ("org", HANMI), ("ent", P, "emi shield 장비", "EMI Shield 장비"), 0.9),
        ],
    },
    # 연구개발 실적 표
    ...
    # 주요매출처: SK하이닉스, Micron, ASE, Amkor, JCET, Infineon, ST Micro, 삼성전기, 삼성전자, LG이노텍 등
    "3c44b8773b46dbbb": {
        "entities": [],
        "edges": [
...
def run():
    ...
    print(f"=== 한미반도체 2025 1분기 ({RCEPT}) 비정형 추출 결과 ===")
