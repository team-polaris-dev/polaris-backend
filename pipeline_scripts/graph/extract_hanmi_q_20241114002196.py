"""비정형 관계 추출 적재 — 한미반도체 2024년 3분기 분기보고서 (rcept 20241114002196, 265청크).

원장은 rcept 전용 ledger/20241114002196.jsonl 에만. 시작 시 기처리 청크 스킵,
대상 청크 전부 mark_processed(엣지 0개여도) → 누락 0. 정형 재무표 제외, 본문 근거 비정형만.

실행: cd db && PYTHONIOENCODING=utf-8 uv run python graph/extract_hanmi_q_20241114002196.py
"""
...
EXTRACTIONS: dict[str, dict] = {
    ...
    # DUAL TC BONDER(2017 SK하이닉스), 6-SIDE INSPECTION, FLIP CHIP/MULTI DIE BONDER
    "2ba0d39c9143a429": {
        "entities": [
    ...
    # 6-SIDE/Flip Chip/MULTI DIE/BIG DIE, micro SAW/VISION PLACEMENT 세계1위
    "2d463dab2d1aa610": {
        "entities": [],
        "edges": [
            E("PRODUCES", ("org", HANMI), ("ent", P, "hbm 6-side inspection", "HBM 6-SIDE INSPECTION"), 0.88),
...
def run():
    ...
    print(f"=== 한미반도체 2024 3분기 ({RCEPT}) 비정형 추출 결과 ===")
