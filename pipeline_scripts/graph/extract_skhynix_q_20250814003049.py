"""비정형 관계 추출 적재 — SK하이닉스 2025년 반기보고서 (rcept 20250814003049, 267청크).

EXTRACTIONS = Claude(에이전트)가 대상 rcept 의 청크 본문을 하나씩 읽고 본문 근거로
판단한 엔티티·엣지. 적재는 extract_helpers 의 멱등 헬퍼로 수행.
원장은 rcept 전용 ledger/20250814003049.jsonl 에만 기록. 정형 표(재무·지분·계열사) 제외, 본문 근거 비정형만.

실행: cd db && PYTHONIOENCODING=utf-8 uv run python graph/extract_skhynix_q_20250814003049.py
"""
...
EXTRACTIONS: dict[str, dict] = {
    ...
    "945e18c27fbd834f": {  # 회사 개요: 메모리 반도체(DRAM/NAND Flash) 주력, Foundry 병행
        "entities": [
    ...
    # ── 매출 품목 표: DRAM, NAND Flash (주요상표 SK하이닉스) ────
    "8a72051440a299db": {  # 매출 품목 표: DRAM, NAND Flash 등 / 주요상표 SK하이닉스
        "entities": [],
        "edges": [
    ...
    "90c97e5ffdecfa66": {  # 메모리 반도체: 당사는 DRAM과 플래시메모리 생산
        "entities": [],
        "edges": [
            E("PRODUCES", ("org", SKH), ("ent", P, "dram", "DRAM"), 0.9),
            E("PRODUCES", ("org", SKH), ("ent", P, "nand flash", "NAND Flash"), 0.9),
    ...
    "4eb0eb2a22b86e49": {  # 시스템반도체 CIS 생산(2025.3 AI메모리로 전환)
        "entities": [
    ...
    # ── 연구개발 실적 표: UFS, Automotive SSD, CMM-DDR5(CXL DRAM), LPDDR5X ─
    "05a177eb0503bd69": {  # R&D 실적: Mobile UFS, Automotive PCIe Gen4 SSD, CMM-DDR5(CXL), 1cnm LPDDR5X
        "entities": [
            (P, "ufs", "UFS"),
            (P, "ssd", "SSD"),
            (P, "cmm-ddr5", "CMM-DDR5"),
            (T, "cxl", "CXL"),
            (P, "lpddr5x", "LPDDR5X"),
            (T, "pcie gen4", "PCIe Gen4"),
        ],
        "edges": [
            E("PRODUCES", ("org", SKH), ("ent", P, "ufs", "UFS"), 0.9),
            E("PRODUCES", ("org", SKH), ("ent", P, "ssd", "SSD"), 0.88),
            E("PRODUCES", ("org", SKH), ("ent", P, "cmm-ddr5", "CMM-DDR5"), 0.88),
            E("USES_TECH", ("org", SKH), ("ent", T, "cxl", "CXL"), 0.88),
            E("PRODUCES", ("org", SKH), ("ent", P, "lpddr5x", "LPDDR5X"), 0.88),
            E("USES_TECH", ("org", SKH), ("ent", T, "pcie gen4", "PCIe Gen4"), 0.85),
        ],
    },
    # ── HBM/GDDR 시황 + 콘솔 협력 ──────────────────────────────
    "74652dc85fc25554": {  # 그래픽스 GDDR, 게임콘솔, HBM
        "entities": [
            (P, "gddr", "GDDR"),
            (P, "hbm", "HBM"),
        ],
        "edges": [
            E("PRODUCES", ("org", SKH), ("ent", P, "gddr", "GDDR"), 0.88),
            E("PRODUCES", ("org", SKH), ("ent", P, "hbm", "HBM"), 0.9),
        ],
    },
    # ── 모바일 메모리: LPDDR5X/T 라인업 ────────────────────────
    "b2ee510bf54241df": {  # 모바일 메모리: 플래그십향 LPDDR5X/T
        "entities": [
            (P, "lpddr5t", "LPDDR5T"),
        ],
        "edges": [
    ...
    # ── 자동차용 메모리 라인업: DDR4/5, LPDDR4/5, Automotive HBM ─
    "db7b9d056effb3b1": {  # Automotive: 범용 DDR4/5, LPDDR4/5, Automotive Grade & HBM
        "entities": [
            (P, "ddr5", "DDR5"),
            (P, "lpddr5", "LPDDR5"),
        ],
    ...
    # ── NAND 경쟁력: QLC, eSSD, Solidigm 통합 시너지 ───────────
    "29429ea546c7fe3f": {  # QLC, PCIe Gen5, 엔터프라이즈 SSD, Solidigm 통합 시너지
        "entities": [
            (P, "qlc", "QLC"),
        ...
        "edges": [
            E("PRODUCES", ("org", SKH), ("ent", P, "qlc", "QLC"), 0.85),
    ...
    # ── 경쟁요소: TSV 기술 ─────────────────────────────────────
    "eb997cba4e187842": {  # DRAM TSV(Through Silicon Via) 기술 경쟁력
        "entities": [
            (T, "tsv", "TSV(Through Silicon Via)"),
        ],
        "edges": [
            E("USES_TECH", ("org", SKH), ("ent", T, "tsv", "TSV(Through Silicon Via)"), 0.85),
        ],
    },
    # ── HBM/ASIC 협력 ──────────────────────────────────────────
    "fe714b721262e33d": {  # HBM 수요, ASIC 개발 협력 강화
        "entities": [],
        "edges": [
    ...
    "77d15baf581588d1": {  # 주요계약: Rambus 특허 크로스라이선스, Intel NAND 영업양수
        "entities": [],
        "edges": [
    ...
    # ── X. 대주주 영업거래: 해외판매·생산법인 (계열 거래) ──────
    "abcd0080bd0b492c": {  # 영업거래: SK hynix America / Wuxi Sales / China
        "entities": [],
        "edges": [
            ...
            E("RELATED_PARTY", ("org", SKH), ("org", "SK hynix Semiconductor China"), 0.85, "해외생산법인(영업거래)"),
        ],
    },
}
...
def run():
    ...
    print(f"=== SK하이닉스 2025 반기 ({RCEPT}) 비정형 추출 결과 ===")
