"""비정형 관계 추출 적재 — 삼성전자 2025년 3분기 분기보고서 (rcept 20251114002447, 228청크).

EXTRACTIONS = Claude(에이전트)가 대상 rcept 의 청크 본문을 하나씩 읽고 본문 근거로
판단한 엔티티·엣지. 적재는 extract_helpers 의 멱등 헬퍼로 수행.
원장은 rcept 전용 ledger/20251114002447.jsonl 에만 기록. 정형 표 제외, 본문 근거 비정형만.

실행: cd db && PYTHONIOENCODING=utf-8 uv run python graph/extract_samsung_q_20251114002447.py
"""
...
EXTRACTIONS: dict[str, dict] = {
    ...
    "36a0eb3d2b7023f3": {  # 회사 개요: DX/DS/SDC/Harman 제품군
        "entities": [
    ...
    "67e19b3d7710c14a": {  # 주요제품 매출: 완제품 + 반도체 + OLED패널 + Harman 디지털콕핏/카오디오/포터블스피커
        "entities": [],
        "edges": [
            E("PRODUCES", ("org", SAMSUNG), ("ent", P, "디지털 콕핏", "디지털 콕핏"), 0.88),
            E("PRODUCES", ("org", SAMSUNG), ("ent", P, "카오디오", "카오디오"), 0.86),
    ...
    "29db63ea5d04de55": {  # Galaxy Z Flip7, Galaxy S25, Galaxy AI, One UI 7, ProVisual, 태블릿/스마트워치/스마트링/무선이어폰
        "entities": [
    ...
    "1f9f9557e3294c32": {  # Galaxy Ecosystem, Samsung Wallet, Samsung Health, Galaxy S25 재활용소재
        "entities": [
    ...
    "e189be3d63f34336": {  # 연구개발 표: Galaxy Z Fold7/Flip7, S25, A, 북, Neo QLED 8K
        "entities": [
            (P, "galaxy z 폴드7", "Galaxy Z Fold7"),
            (P, "galaxy z 플립7", "Galaxy Z 플립7"),
            (P, "neo qled", "Neo QLED"),
            (P, "galaxy 북", "Galaxy 북"),
        ],
        "edges": [
            E("PRODUCES", ("org", SAMSUNG), ("ent", P, "galaxy z 폴드7", "Galaxy Z Fold7"), 0.92),
            E("PRODUCES", ("org", SAMSUNG), ("ent", P, "galaxy z 플립7", "Galaxy Z 플립7"), 0.92),
    ...
    "79167d8d724fe758": {  # AI TV: Neo QLED, OLED, QLED, 더 프레임
        "entities": [
            (P, "qled", "QLED"),
    ...
    "7e9498f2ad814f2c": {  # TV 기술: 마이크로 LED, AI 화질 기술
        "entities": [
            (T, "마이크로 led", "마이크로 LED"),
            (T, "ai 화질 기술", "AI 화질 기술"),
        ],
    ...
    "cadf80e133d4bcc2": {  # Q시리즈 사운드바, 더 프레임, Neo QLED/QLED, 더 프리미어 5
        "entities": [
            (P, "사운드바", "사운드바"),
            (P, "프로젝터", "프로젝터"),
        ],
    ...
    "ffa56e5427cac8f4": {  # System LSI 모바일AP/이미지센서, Foundry, HBM/DDR5/SSD
        "entities": [
            (P, "이미지센서", "이미지 센서"),
            (T, "foundry", "Foundry"),
        ],
    ...
    "9e20eb1788b41b46": {  # 메모리: HBM3E, DDR5, NAND, 서버 SSD
        "entities": [
            (P, "hbm3e", "HBM3E"),
            (P, "ddr5", "DDR5"),
        ],
    ...
    "3e31cab7bb6a0d21": {  # Power 서버향, Foundry 2나노 1세대/2세대, 4나노, OLED/QD-OLED/TFT-LCD
        "entities": [
            (T, "2나노 공정", "2나노 공정"),
            (T, "qd-oled", "QD-OLED"),
            (T, "oled", "OLED"),
    ...
    "a0e1a80d7604e2bf": {  # 원재료: 모바일AP/Camera ← Qualcomm/삼성전기, 패널 ← AUO, Chemical/Wafer ← 솔브레인/SILTRONIC, FPCA/Cover Glass ← 비에이치/Apple, SOC/통신모듈 ← NVIDIA/WNC
        "entities": [],
        "edges": [
    ...
    "f30f4b2223ee504e": {  # 주요 매입처 표: Qualcomm/MediaTek, AUO/CSOT, 삼성전기/엠씨넥스, 솔브레인/동우화인켐, SILTRONIC/SK실트론
        "entities": [],
        "edges": [
    ...
    "9d78dcf8e3ad60e7": {  # DS Foundry — Tesla 반도체 위탁생산 수주(16,544백만달러)
        "entities": [],
        "edges": [
    ...
    "bc03109be806a289": {  # 특허 라이선스: Google, Nokia, Qualcomm, Huawei
        "entities": [],
        "edges": [
    ...
    "4334c42af670b325": {  # 경영상 주요계약 표: Google/Ericsson/Qualcomm/Huawei 상호특허
        "entities": [],
        "edges": [
    ...
    "3ab055f38f4b364c": {  # Harman, Sound United(B&W/Denon/Marantz) 2025 3분기 인수
        "entities": [],
        "edges": [
            E("RELATED_PARTY", ("org", "Harman"), ("org", "Sound United"), 0.88, "인수(2025)"),
        ],
    },
    "5777a3859668cf7e": {  # 레인보우로보틱스 콜옵션 행사완료, 삼성디스플레이-Corning/TCL/CSOT 풋옵션
        "entities": [],
        "edges": [
    ...
    "fb19963883b46181": {  # 해외 종속: SEA, SII, SSI, SAS, SEDA, Harman
        "entities": [],
        "edges": [
    ...
    "b7db83a33f92fca8": {  # 채무보증 SEA, 자산양수도 SCS, 영업거래 SSI
        "entities": [],
        "edges": [
...
def run():
    ...
    print(f"=== 삼성전자 2025 3분기 ({RCEPT}) 비정형 추출 결과 ===")
