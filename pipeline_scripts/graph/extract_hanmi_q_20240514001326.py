"""비정형 관계 추출 적재 — 한미반도체 2024년 1분기 분기보고서 (rcept 20240514001326, 253청크).

Claude(에이전트)가 대상 rcept 청크 본문을 읽고 본문 근거로 판단한 엔티티·엣지를
extract_helpers 멱등 헬퍼로 적재. 원장은 rcept 전용 ledger/20240514001326.jsonl 에만.
시작 시 그 원장을 확인해 기처리 청크 스킵, 대상 청크 전부 mark_processed(엣지 0개여도) → 누락 0.
정형 재무제표 표는 제외, 본문 근거 있는 비정형만.

실행: cd db && PYTHONIOENCODING=utf-8 uv run python graph/extract_hanmi_q_20240514001326.py
"""
...
EXTRACTIONS: dict[str, dict] = {
    ...
    # 시장여건·경쟁상황: 주력장비 VISION PLACEMENT/micro SAW, DUAL TC Bonder(2017 SK하이닉스 공동개발)
    "bcbe1a5e1f4216c1": {
        "entities": [
        ...
        "edges": [
            E("PRODUCES", ("org", HANMI), ("ent", P, "vision placement", "VISION PLACEMENT"), 0.95),
    ...
    # 제조용 장비 라인업 나열 (DUAL TC BONDER, FLIP CHIP BONDER, EMI Shield, META GRINDER, TAPE SAW, Wafer SAW 등)
    "7fb709451f577871": {
        "entities": [
    ...
    # DUAL TC BONDER(2017 SK하이닉스 공동개발), FLIP CHIP/MULTI DIE/BIG DIE BONDER, EMI Shield
    "c7aaed702c0614a1": {
        "entities": [
    ...
    # TC Bonder HBM 핵심장비, Flip Chip Bonder, EMI Shield 세계 1위
    "b3f9ff57f8bf4405": {
        "entities": [],
        "edges": [
    ...
    # 연구개발 실적 표: micro SAW 시리즈, EMI Shield, TC BONDER(SK하이닉스 공동개발), FLIP CHIP BONDER, META GRINDER, LASER MARKING/CUTTING, Pick & Place
    "f1d2c26ca818ea99": {
        "entities": [
    ...
    # 주요매출처(고객사): ASE, Amkor, Micron, Infineon, ST Micro, SK하이닉스, 삼성전기, 삼성전자, LG이노텍 등 → 한미가 장비 공급
    "75f25de012afa2e6": {
        "entities": [],
        "edges": [
...
def run():
    ...
    print(f"=== 한미반도체 2024 1분기 ({RCEPT}) 비정형 추출 결과 ===")
